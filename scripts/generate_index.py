#!/usr/bin/env python3
"""Generate index.html listing all ADHD daily reports."""

import glob
import os
from datetime import datetime

html_files = sorted(glob.glob("docs/adhd-*.html"), reverse=True)
links = ""
for f in html_files[:30]:
    name = os.path.basename(f)
    date = name.replace("adhd-", "").replace(".html", "")
    try:
...
<title>ADHD Brain · ADHD 文獻日報</title>
<style>
  :root {{ --bg: #f0f4ff; --surface: #f8faff; --line: #c8d6e5; --text: #1e2a3a; --muted: #5a6d82; --accent: #2d6cdf; --accent-soft: #d4e2fc; }}
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: radial-gradient(circle at top, #e8f0ff 0, var(--bg) 55%, #d0dff5 100%); color: var(--text); font-family: "Noto Sans TC", "PingFang TC", "Helvetica Neue", Arial, sans-serif; min-height: 100vh; }}
  ...
  .clinic-link {{ display: flex; align-items: center; gap: 10px; padding: 12px 18px; background: var(--surface); border: 1px solid var(--line); border-radius: 12px; text-decoration: none; color: var(--text); font-size: 14px; transition: all 0.2s; }}
...
<div class="container">
  <div class="logo">🧠</div>
  <h1>ADHD Brain</h1>
  <p class="subtitle">ADHD 文獻日報 · 每日自動更新</p>
